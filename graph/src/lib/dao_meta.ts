...
export default function daoMeta(
    ...
    if(outcome.logs.length > 0) {
      ...
      if (jsonObject) {  
        ...
        if(action.toString() == "Finalize" || action.toString() == "VoteApprove" || action.toString() == "VoteReject" || action.toString() == "Approved") {
          let idVote = id.toString() +"|"+ sender_id.toString() +"|"+ blockHeader.timestampNanosec.toString();
          let vote = Vote.load(idVote);

          let proposal = Proposal.load(id.toString());
          if (proposal) {
            if(proposal.status == "InProgress" && status.toString() != "InProgress" /*&& status.toString() != "Failed"*/) {
              let proposaldata = Proposaldata.load("1");
              if(proposaldata) {
            ...
            if(!vote) {
              ...
              let owner = Owner.load(sender_id.toString())
              if(owner) {
                owner.votes = owner.votes.plus(BigInt.fromI32(1));
